...
(function ($, document) {
    $(document).ready(function(){
		...
		var re_group = new RegExp(/^\/groups.*$/g);
		var re_event = new RegExp(/^\/events.*$/g);
		...
		// on video clear
		// <span>Video Link:</span><input name="video-link" type="text" id="post-video-text" value="http://" class="jq-inputbox-video user-input"/>
		
		//$('.post-button button').click(function() {
		    
		    // gather the content from the necessary files    
		    //var post_content = $('textarea.jq-textarea-comment').val().trim();
		    
		    //if(post_content!="" || post_content!="Type your comment here.")
		    //{
		      //  if(re_group.test($.trim(window_path)))
			//{
			    // group, post to group
			//}
			//else if(re_event.test($.trim(window_path)))
		       // {
			    // if event, post to event 
		       // }
			//else
			//{
			    // otherwise the default is to post the content
			    // to the active user's profile
			    //var dataString = "content="+post_content+"&link_type="+content_type+"&link="+content_link;
			    
			    //$.ajax({
			      //  method: 'POST',
			       // url: '/social_utility/postContent',
				//data: dataString,
				//success: function(data)
				//{
		    //		$('.post-container').empty();
		    //		$('.post-container').html("Posted successfully!");
		    //	    },
		    //	    error: function(data)
		    //	    {
		    //		$('.post-container').append("<div>Oops, something wrong happened.</div>");
		    //	    }
		    //	});
		      //  }
		    //}
